...
export const buildWorkoutCsv = (
  ...
  const rows = sortedRecords.map((record) => {
    ...
    return [
      ...
      exercise?.exerciseId ?? record.exerciseId,
      exercise?.baseExerciseId ?? exercise?.exerciseId ?? record.exerciseId,
      exercise?.name ?? record.exerciseId,
      exercise?.baseExerciseName ?? exercise?.name ?? record.exerciseId,
